...
export default function BudgetPage() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      const { data: t, error: txErr } = await supabase
        .from("transactions")
        .select("id, category_id, credit_card_id, amount, date, name")
        .eq("user_id", u.user.id)
        .gte("date", start)
        .lt("date", end);
    ...
    return (
      <table className="w-full border-collapse text-sm">
        ...
        <tbody className="text-zinc-900 dark:text-zinc-100">
          {rows.length === 0 ? (
            <tr>
              <td className="p-2 text-zinc-600 dark:text-zinc-300" colSpan={showOrder ? 5 : 4}>
                Nothing here yet.
              </td>
            </tr>
          ) : (
                  ...
                  <td className="p-2 text-right">
                    {r.orderableCategoryId ? (
                      <div className="flex items-center justify-end gap-1">
                        <button
                          onClick={() => onMove(r.orderableCategoryId!, -1)}
                ...
                <td className="p-2 text-right tabular-nums">
                  {r.editable === false ? (
                    ...
                    <div className="flex items-center justify-end gap-2">
                      ...
                      />
                      <button
                        onClick={() => updatePlannedTotal(r.id)}
                        className="rounded-md border border-zinc-300 bg-white px-2 py-1 text-xs hover:bg-zinc-100 dark:border-zinc-700 dark:bg-zinc-950 dark:hover:bg-zinc-900"
                      >
                        Save
                      </button>
                    ...
                    <button
                      onClick={() => {
                        setEditPlannedKey(r.id);
                        setEditPlannedAmount(String(r.planned));
                        onManage(r.id);
                      }}
  ...
  const expenseRows = buildCategoryRows(expenseCats);
